...
class BugsnagNotifier {
  ...
  /// ```
  init(apiKey, {String releaseStage = 'production'}) {
    this._apiKey = apiKey;
  ...
  List<Map<String, String>> _parseStackTrace(StackTrace stackTrace) {
    ...
    for (var line in stackTraceLines) {
      ...
      stackTraceObjects.add({
        'file': splitStackTraceLine.length >= 4 ? splitStackTraceLine[3].toString() : '',
        'lineNumber': splitStackTraceLine.length >= 5 ? splitStackTraceLine[4].toString() : '',
        'columnNumber': splitStackTraceLine.length >= 6 ? splitStackTraceLine[5].toString() : '',
        'method':
            splitStackTraceLine.length >= 2 ? splitStackTraceLine[1].toString() : splitStackTraceLine[0].toString(),
      });
      // }
    }
