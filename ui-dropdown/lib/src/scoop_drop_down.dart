  ...
  void _handleFocusChange(bool focused) {
    ...
    if (focused && inTraditionalMode) {
      final _MenuLimits menuLimits =
          widget.route.getMenuLimits(widget.buttonRect, widget.constraints.maxHeight, widget.itemIndex);
...
class _DropdownRoute<T> extends PopupRoute<_DropdownRouteResult<T>> {
  ...
  _MenuLimits getMenuLimits(Rect buttonRect, double availableHeight, int index) {
    ...
    final double selectedItemOffset = 20.0;
    ...
    // (the default).
    final double scrollOffset =
        preferredMenuHeight <= maxMenuHeight ? 0 : math.max(0.0, selectedItemOffset - (buttonTop - menuTop));
...
class _DropdownButtonState<T> extends State<CustomDropdownButton<T>> with WidgetsBindingObserver {
  ...
  TextStyle get _textStyle => widget.style ?? MyTheme.dropdownElement;
  ...
  double get _denseButtonHeight {
    final double fontSize = _textStyle.fontSize ?? MyTheme.textFontSize;
  ...
  Widget build(BuildContext context) {
    ...
    if (_enabled) {
      items =
          widget.selectedItemBuilder == null ? List<Widget>.from(widget.items) : widget.selectedItemBuilder!(context);
    } else {
    ...
    Widget result = DefaultTextStyle(
      style: _textStyle,
      child: Container(
        decoration: _showHighlight == true
            ? BoxDecoration(
